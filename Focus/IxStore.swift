...
public struct IxStore<O, I, A> {
	...
	public func map<B>(_ f : (A) -> B) -> IxStore<O, I, B> {
		return f <^> self
	}

	/// Applies a function to the position index.
	public func imap<P>(_ f : (O) -> P) -> IxStore<P, I, A> {
		return f <^^> self
	}

	/// Applies a function to the retrieval index.
	public func contramap<H>(_ f : (H) -> I) -> IxStore<O, H, A> {
		return f <!> self
	}
	...
	public func extend<E, B>(_ f : (IxStore<E, I, A>) -> B) -> IxStore<O, E, B> {
		return self ->> f
	}
